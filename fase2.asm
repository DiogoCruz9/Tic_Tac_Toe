...
#Abra o Bitmap Display para ver o jogo 
...
.data	
	...
	invalida: .asciiz "Você não pode jogar nessa posição, já está ocupada\n"
...
cores:
	addi $s0, $zero, 0xffffff #Branco
	addi $s1, $zero, 0xFF0000 #Vermelho
	addi $s2, $zero, 0x3EDD02 #Verde
...
jogada_player1:
		la $a0, salta_linha	#Mensagem para escolher jogada	
		li $v0, 4
		syscall	
		la $a0, jogar
		li $v0, 4
...
verifica_vitoria:
			...
			lw $t8, 20($t0)
			lw $t9, 32($t0)
			beq $s3, $t4, emp	#Verifica se é diferente de preto
			bne $t4, $t8, emp	# 3 == 6
			beq $t4, $t9, fim	# 3 == 9
...
fim_empate:
			la $a0, msg_empate		#Printa que deu empate
			li $v0, 4
			syscall
